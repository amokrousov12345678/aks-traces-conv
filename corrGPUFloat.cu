//nvcc corrGPUFloat.cu -lcufft --compiler-options -fPIC -std=c++11
...
__global__ void ComplexPointwiseMulAndScale(cufftComplex *c, cufftComplex *a, const cufftComplex *b, int size, float scale) {
  ...
  for (int i = threadID; i < size; i += numThreads) {
    tmp = cuCmulf(a[i], b[i]);
    c[i] = make_cuFloatComplex(scale*cuCrealf(tmp), scale*cuCimagf(tmp));
  }
}
...
void calcCorrelationFFT(float *S, int N, int L, float *X, float* W) {
    ...
    float* a = (float*) calloc(arraySize, sizeof(float));
    memcpy(a, S, N * sizeof(float));
    std::reverse(a + 1, a + arraySize);

    cufftReal* dev_a;
    checkCudaErrors(cudaMalloc((void**)&dev_a, arraySize * sizeof(cufftReal)));
    checkCudaErrors(cudaMemcpy(dev_a, a, arraySize * sizeof(float), cudaMemcpyHostToDevice));
    free(a);

    float* b = (float*) calloc(arraySize, sizeof(float));
    memcpy(b, X, (L+N) * sizeof(float));

    cufftReal* dev_b;
    checkCudaErrors(cudaMalloc((void**)&dev_b, arraySize * sizeof(cufftReal)));
    checkCudaErrors(cudaMemcpy(dev_b, b, arraySize * sizeof(float), cudaMemcpyHostToDevice));
    free(b);

    cufftHandle plan;
    checkCufftErrors(cufftPlan1d(&plan, arraySize, CUFFT_R2C, 1));

    cufftComplex* dev_a_fft;
    checkCudaErrors(cudaMalloc((void**)&dev_a_fft, complexSize * sizeof(cufftComplex)));
    checkCufftErrors(cufftExecR2C(plan, dev_a, dev_a_fft));
    checkCudaErrors(cudaDeviceSynchronize());
    checkCudaErrors(cudaFree(dev_a));

    cufftComplex* dev_b_fft;
    checkCudaErrors(cudaMalloc((void**)&dev_b_fft, complexSize * sizeof(cufftComplex)));
    checkCufftErrors(cufftExecR2C(plan, dev_b, dev_b_fft));
    checkCudaErrors(cudaDeviceSynchronize());
    checkCudaErrors(cudaFree(dev_b));

    checkCufftErrors(cufftDestroy(plan));

    cufftComplex* dev_c_fft;
    checkCudaErrors(cudaMalloc((void**)&dev_c_fft, complexSize * sizeof(cufftComplex)));
    ComplexPointwiseMulAndScale<<<32, 256>>>(dev_c_fft, dev_a_fft, dev_b_fft, complexSize, 1./arraySize);
    checkCudaErrors(cudaDeviceSynchronize());
    checkCudaErrors(cudaFree(dev_a_fft));
    checkCudaErrors(cudaFree(dev_b_fft));
    
    cufftHandle planInv;
    checkCufftErrors(cufftPlan1d(&planInv, arraySize, CUFFT_C2R, 1));

    cufftReal* dev_c;
    checkCudaErrors(cudaMalloc((void**)&dev_c, arraySize * sizeof(cufftReal)));
    checkCufftErrors(cufftExecC2R(planInv, dev_c_fft, dev_c));
    checkCudaErrors(cudaDeviceSynchronize());
    checkCudaErrors(cudaFree(dev_c_fft));

    checkCufftErrors(cufftDestroy(planInv));
    
    checkCudaErrors(cudaMemcpy(W, dev_c, L * sizeof(float), cudaMemcpyDeviceToHost));
