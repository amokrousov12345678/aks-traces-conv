//nvcc corrGPUFloat2in1.cu -lcufft --compiler-options -fPIC -std=c++11
...
void calcCorrelationFFT(float *S, int N, int L, float *X, float* W) {
    ...
    cufftReal* dev_ab;
    checkCudaErrors(cudaMalloc((void**)&dev_ab, 2 * arraySize * sizeof(cufftReal)));
    checkCudaErrors(cudaMemcpy(dev_ab, a, arraySize * sizeof(float), cudaMemcpyHostToDevice));
    free(a);

    float* b = (float*) calloc(arraySize, sizeof(float));
    memcpy(b, X, (L+N) * sizeof(float));

    checkCudaErrors(cudaMemcpy(dev_ab + arraySize, b, arraySize * sizeof(float), cudaMemcpyHostToDevice));
    free(b);

    cufftHandle plan;
    checkCufftErrors(cufftPlan1d(&plan, arraySize, CUFFT_R2C, 2));

    cufftComplex* dev_ab_fft;
    checkCudaErrors(cudaMalloc((void**)&dev_ab_fft, 2 * complexSize * sizeof(cufftComplex)));
    checkCufftErrors(cufftExecR2C(plan, dev_ab, dev_ab_fft));
