//nvcc corrGPUDouble.cu -lcufft --compiler-options -fPIC -std=c++11
...
void calcCorrelationFFT(float *S, int N, int L, float *X, float* W) {
    ...
    cufftDoubleReal* dev_a;
    checkCudaErrors(cudaMalloc((void**)&dev_a, arraySize * sizeof(cufftDoubleReal)));
    checkCudaErrors(cudaMemcpy(dev_a, a, arraySize * sizeof(double), cudaMemcpyHostToDevice));
    free(a);
    ...
    cufftDoubleReal* dev_b;
    checkCudaErrors(cudaMalloc((void**)&dev_b, arraySize * sizeof(cufftDoubleReal)));
    checkCudaErrors(cudaMemcpy(dev_b, b, arraySize * sizeof(double), cudaMemcpyHostToDevice));
    free(b);

    cufftHandle plan;
    checkCufftErrors(cufftPlan1d(&plan, arraySize, CUFFT_D2Z, 1));

    cufftDoubleComplex* dev_a_fft;
    checkCudaErrors(cudaMalloc((void**)&dev_a_fft, complexSize * sizeof(cufftDoubleComplex)));
    checkCufftErrors(cufftExecD2Z(plan, dev_a, dev_a_fft));
    checkCudaErrors(cudaDeviceSynchronize());
    checkCudaErrors(cudaFree(dev_a));

    cufftDoubleComplex* dev_b_fft;
    checkCudaErrors(cudaMalloc((void**)&dev_b_fft, complexSize * sizeof(cufftDoubleComplex)));
    checkCufftErrors(cufftExecD2Z(plan, dev_b, dev_b_fft));
    checkCudaErrors(cudaDeviceSynchronize());
